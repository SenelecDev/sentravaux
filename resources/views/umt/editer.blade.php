...
@section('title', 'Gérer la demande - UMT')
...
<div class="max-w-7xl mx-auto space-y-8" x-data="{
    ...
    <div class="flex items-center gap-4 bg-white rounded-xl shadow-sm px-6 py-4 border border-gray-100">
        <a href="{{ route('umt.demandes.recues') }}" class="p-2 text-gray-500 hover:text-gray-700 hover:bg-gray-100 rounded-lg transition-colors">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/></svg>
        </a>
        <div class="flex-1">
            <h1 class="text-2xl font-bold text-gray-900">Gérer la demande {{ $demande->numero_demande }}</h1>
            <p class="mt-1 text-gray-500">Unité Maintenance Thermique (UMT) &middot; {{ $demande->objet ?? '-' }}</p>
    ...
    @php
        ...
        // Formulaire en lecture seule si:
        // - la demande est terminée
        // - OU l'utilisateur est chef d'équipe et la demande n'est ni "valide" ni "en_cours"
        $isFormReadOnly = $statutLower === 'termine'
            || (auth()->user()->hasRole('equipe') && !in_array($statutLower, ['valide', 'en_cours']));
    @endphp
    ...
    <form method="POST" action="{{ route('umt.update', $demande) }}" id="editForm" class="space-y-6">
        @csrf
        @method('PUT')

        <fieldset @if($isFormReadOnly) disabled @endif class="space-y-6">
                ...
                <div class="p-6">
                    <div class="grid gap-6"
                        ...
                        <div x-show="teamType === 'interne'">
                            ...
                            <select name="chef_equipe_id" class="select2-users" @if($isFormReadOnly) disabled @endif>
                        ...
                        <div x-show="teamType === 'interne'">
                            ...
                            <select name="superviseur_id" class="select2-users" @if($isFormReadOnly) disabled @endif>
                                <option value="">-- Sélectionner --</option>
                                @foreach($users as $user)
                        ...
                        <div x-show="teamType === 'externe'">
                            <label class="block text-sm font-medium text-gray-700 mb-1">Superviseur</label>
                            <select name="superviseur_externe_id" class="select2-users" @if($isFormReadOnly) disabled @endif>
                                <option value="">-- Sélectionner --</option>
                                @foreach($users as $user)
                        ...
                        <div x-show="teamType === 'externe'">
                            ...
                            <input type="text" name="prestataire_nom" value="{{ old('prestataire_nom', $demande->prestataire_nom) }}" class="input-senelec w-full" placeholder="Nom du prestataire" @if($isFormReadOnly) disabled @endif>
                        </div>

                        {{-- Exécutant (interne uniquement) --}}
                        <div x-show="teamType === 'interne'">
                            <label class="block text-sm font-medium text-gray-700 mb-1">Exécutant</label>
                            <select name="executant_id" class="select2-users" @if($isFormReadOnly) disabled @endif>
                                <option value="">-- Sélectionner --</option>
                                @foreach($users as $user)
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 p-4 bg-gray-50 rounded-lg">
                        ...
                        <div>
                            ...
                            <input type="number"
                                   ...
                                   @if($isFormReadOnly) disabled @endif>
                        </div>
                        <div>
                            <label class="block text-xs font-medium text-gray-500 mb-1">Exécutant</label>
                            <select name="executant_equipe[{{ $index }}]" class="select2-users" @if($isFormReadOnly) disabled @endif>
            ...
            <div class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div>
                        ...
                        <input type="text" name="numero_commande" value="{{ old('numero_commande', $demande->numero_commande) }}" class="input-senelec w-full" placeholder="Numéro de commande" @if($isFormReadOnly) disabled @endif>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Commentaire prestataire</label>
                        <input type="text" name="commentaire_prestataire" value="{{ old('commentaire_prestataire', $demande->commentaire_prestataire) }}" class="input-senelec w-full" placeholder="Commentaire" @if($isFormReadOnly) disabled @endif>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="card-senelec">
            ...
            <div class="p-6">
                <textarea name="commentaire" rows="3" class="input-senelec w-full" placeholder="Ajouter un commentaire..." @if($isFormReadOnly) disabled @endif>{{ old('commentaire', $demande->comment_umt) }}</textarea>
        ...
        <div class="mt-6">
            <div class="flex flex-col md:flex-row gap-3">
                @if($demande->statut !== 'termine')
                    ...
                    {{-- Valider & dispatcher (UMT) --}}
                    @if(in_array($demande->statut, ['impute', 'accepte']) && auth()->user()->hasRole('umt'))
                ...
                {{-- Clôture --}}
                @if($demande->statut === 'termine' && auth()->user()->hasRole('umt'))
                    <button type="button"
    ...
    <div x-show="showTerminerModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div class="relative bg-white rounded-xl shadow-xl max-w-md w-full p-6 z-10" @click.stop>
            ...
            <form method="POST" action="{{ route('umt.update', $demande) }}">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-4">
                        ...
                        <div x-show="teamType === 'externe'">
                            ...
                            <input type="text" name="prestataire_nom" value="{{ $demande->prestataire_nom }}" class="input-senelec w-full" placeholder="Nom du prestataire">
                        </div>
                        <div x-show="teamType === 'externe'">
                            <label class="block text-sm font-medium text-gray-700 mb-1">N° commande</label>
                            <input type="text" name="numero_commande" value="{{ $demande->numero_commande }}" class="input-senelec w-full" placeholder="Numéro de commande">
                        </div>
                </div>
