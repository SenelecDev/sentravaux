...
<div class="max-w-7xl mx-auto space-y-8" x-data="{
    ...
    <div class="flex items-center gap-4 bg-white rounded-xl shadow-sm px-6 py-4 border border-gray-100">
        <a href="{{ route('unsp.demandes.recues') }}" class="p-2 text-gray-500 hover:text-gray-700 hover:bg-gray-100 rounded-lg transition-colors">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/></svg>
        </a>
        <div class="flex-1">
            <h1 class="text-2xl font-bold text-gray-900">Gérer la demande {{ $demande->numero_demande }}</h1>
            <p class="mt-1 text-gray-500">Unité Nettoyage et Salubrité Publique (UNSP) &middot; {{ $demande->objet ?? '-' }}</p>
    ...
    <form method="POST" action="{{ route('unsp.update', $demande) }}" id="editForm" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="card-senelec">
            ...
            <div class="p-6">
                <textarea name="commentaire" rows="3" class="input-senelec w-full" placeholder="Ajouter un commentaire..." @if($isTermine) disabled @endif>{{ old('commentaire', $demande->comment_unsp) }}</textarea>
        ...
        <div class="mt-6">
            <div class="flex flex-col md:flex-row gap-3">
                @if($demande->statut !== 'termine')
                    ...
                    {{-- Valider & dispatcher (UNSP) --}}
                    @if(in_array($demande->statut, ['impute', 'accepte']) && auth()->user()->hasRole('unsp'))
                ...
                {{-- Clôture --}}
                @if($demande->statut === 'termine' && auth()->user()->hasRole('unsp'))
                    <button type="button"
    ...
    <div x-show="showTerminerModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div class="relative bg-white rounded-xl shadow-xl max-w-md w-full p-6 z-10" @click.stop>
            ...
            <form method="POST" action="{{ route('unsp.update', $demande) }}">
                @csrf
                @method('PUT')
