...
class UnspController extends Controller
{
    public function index()
    {
        $unsps = User::role("unsp")->get();
        return view("unsp.index", compact("unsps"));
    }
    ...
    public function edit(string $id)
    {
        ...
        return view('unsp.editer', compact('demande', 'equipes', 'users', 'chefEquipeUsers'));
    }
    ...
    public function update(Request $request, string $id)
    {
        ...
        if ($demande->statut === 'cloture') {
            return redirect()->route('unsp.demandes.recues')->with('error', 'La demande ne peut plus être modifiée.');
        }
        ...
        // On ne force team_type que pour l'action de dispatch
        ...
        // Mapper correctement le commentaire UNSP (champ du formulaire => colonne comment_unsp)
        if ($request->has('commentaire')) {
            $updateData['comment_unsp'] = $request->input('commentaire');
        ...
        switch ($action) {
            case 'dispatcher':
                if ($teamType === 'externe') {
                    ...
                    return redirect()->route('unsp.demandes.validees')->with('success', 'Demande validée pour prestataire externe.');
                }
                ...
                if ($chefEquipeUserId) {
                    ...
                    return redirect()->route('unsp.demandes.validees')->with('success', 'Demande validée et transférée au chef d\'équipe.');
                }
            ...
            case 'retour':
                if ($demande->chef_equipe_id) {
                    ...
                    return redirect()->route('unsp.demandes.recues')->with('success', 'Demande renvoyée au chef d\'équipe.');
                }
            ...
            case 'debut_travaux':
                ...
                return redirect()->route('unsp.demandes.debutees')->with('success', 'Début des travaux enregistré.');
            ...
            case 'terminer':
                ...
                return redirect()->route('unsp.demandes.terminees')->with('success', 'Demande terminée avec succès.');
            ...
            case 'cloturer':
                ...
                try {
                    ...
                    \Log::error('Erreur lors de la génération/envoi du PDF de clôture UNSP', [
                        'demande_id' => $demande->id,
                        'message'    => $e->getMessage(),
                    ]);
                }

                return redirect()->route('unsp.demandes.recues')->with('success', 'Demande clôturée avec succès.');

            default:
                return redirect()->route('unsp.demandes.recues')->with('success', 'Demande mise à jour avec succès.');
        }
    }
    ...
    public function demandesTerminees()
    {
        ...
        $demandes = Demande::where('statut', 'termine')
            ->where(function ($q) use ($user) {
                $q->where('unsp_id', $user->id)
                  ->orWhereHas('unsp', fn($s) => $s->where('id', $user->id));
            })
            ->with(['user', 'site', 'service', 'equipes', 'chefequipe', 'superviseur', 'executant', 'terminatedBy', 'validatedBy', 'unsp'])
            ->orderBy('updated_at', 'desc')
            ->get();
        return view('unsp.demandes.terminees', compact('demandes'));
    }
    ...
    public function demandesCloturees()
    {
        ...
        $demandes = Demande::where('statut', 'cloture')
            ...
            ->with(['user', 'site', 'service', 'equipes', 'chefequipe', 'superviseur', 'executant', 'terminatedBy', 'cloturedBy', 'validatedBy', 'unsp'])
            ->orderBy('updated_at', 'desc')
            ->get();
        return view('unsp.demandes.cloturees', compact('demandes'));
    }
    ...
    public function dashboard(Request $request)
    {
        ...
        $baseQuery = fn() => Demande::where(function ($q) use ($user) { $q->whereNotNull('unsp_id')->orWhere('unsp_id', $user->id); })
        ...
        $travauxDebutes = Demande::where('unsp_id', $user->id)->where('statut', 'en_cours')
            ->with(['user', 'site', 'service', 'chefequipe'])->orderBy('date_debut_intervention', 'desc')->limit(10)->get();

        $periodesAvenir = Demande::where('unsp_id', $user->id)
        ...
        // Compteurs individuels attendus par la vue (comme UBT/UMT)
        ...
        return view('unsp.dashboard', compact(
            'totalDemandes',
            'statsParStatut',
            'demandesParMois',
    ...
    {
        $demandes = Demande::whereNotNull('unsp_id')
            ->where('statut', 'impute')
            ->orderBy('created_at', 'desc')
            ->get();
        foreach ($demandes as $d) $d->statut = ucfirst(str_replace('_', ' ', $d->statut));
        return view('unsp.demande', compact('demandes'));
    }

    public function demandesValidees()
    {
        $demandes = Demande::whereNotNull('unsp_id')->where('statut', 'valide')
            ->with(['user', 'service', 'site'])->orderBy('updated_at', 'desc')->get();
        return view('unsp.demandes_validees', compact('demandes'));
    }

    public function demandesDebutees()
    {
        $user = auth()->user();
        $demandes = Demande::where('unsp_id', $user->id)->where('statut', 'en_cours')
            ->with(['user', 'site', 'service', 'equipes', 'chefequipe', 'superviseur', 'executant'])
            ->orderBy('date_debut_intervention', 'desc')->get();
        return view('unsp.demandes.debutees', compact('demandes'));
    }
}
