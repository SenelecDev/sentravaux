...
{
    public function index() { return view('ucc.index'); }
    ...
    public function dashboard(Request $request)
    {
        ...
        $baseQuery = fn() => Demande::where(function ($q) use ($user) {
            $q->whereNotNull('ucc_id')->orWhere('ucc_id', $user->id);
        ...
        $travauxDebutes = Demande::where('ucc_id', $user->id)->where('statut', 'en_cours')
            ->with(['user', 'site', 'service', 'chefequipe'])->orderBy('date_debut_intervention', 'desc')->limit(10)->get();

        $periodesAvenir = Demande::where('ucc_id', $user->id)
        ...
        return view('ucc.dashboard', compact(
            'totalDemandes',
            'statsParStatut',
            'demandesParMois',
    ...
    public function demandes() { return $this->listByStatus(null, 'Demandes recues UCC'); }
    public function demandesValidees() { return $this->listByStatus('valide', 'Demandes validees UCC'); }
    public function demandesDebutees() { return $this->listByStatus('en_cours', 'Travaux debutes UCC'); }
    public function demandesTerminees() { return $this->listByStatus('termine', 'Demandes terminees UCC'); }
    public function demandesCloturees() { return $this->listByStatus('cloture', 'Demandes cloturees UCC'); }
    ...
    public function edit(string $id)
    {
        ...
        return view('ucc.editer', ['demande' => $demande, 'equipes' => Equipe::all(), 'users' => $users, 'chefEquipeUsers' => $chefEquipeUsers]);
    }
    ...
    public function update(Request $request, string $id)
    {
        ...
        if ($demande->statut === 'cloture') {
            return redirect()->route('ucc.demandes.recues')->with('error', 'La demande ne peut plus être modifiée.');
        }
        ...
        switch ($action) {
            case 'dispatcher':
                if ($teamType === 'externe') {
                    ...
                    return redirect()->route('ucc.demandes.validees')->with('success', 'Demande validée pour prestataire externe.');
                }
                if ($chefEquipeUserId) {
                    ...
                    return redirect()->route('ucc.demandes.validees')->with('success', 'Demande validée et transférée au chef d\'équipe.');
                }
            ...
            case 'retour':
                if ($demande->chef_equipe_id) {
                    ...
                    return redirect()->route('ucc.demandes.recues')->with('success', 'Demande renvoyée au chef d\'équipe.');
                }
            ...
            case 'debut_travaux':
                ...
                return redirect()->route('ucc.demandes.debutees')->with('success', 'Début des travaux enregistré.');
            ...
            case 'terminer':
                ...
                return redirect()->route('ucc.demandes.terminees')->with('success', 'Demande terminée avec succès.');
            ...
            case 'cloturer':
                ...
                return redirect()->route('ucc.demandes.recues')->with('success', 'Demande clôturée avec succès.');

            default:
                return redirect()->route('ucc.demandes.recues')->with('success', 'Demande mise a jour.');
        }
    }

    private function listByStatus(?string $statut, string $title)
    {
        $query = Demande::whereNotNull('ucc_id')->with(['user', 'site', 'service']);
        if ($statut) $query->where('statut', $statut);
        $demandes = $query->latest('updated_at')->get();
        return view('ucc.demande', compact('demandes', 'title'));
    }
}
