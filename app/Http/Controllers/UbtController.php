...
class UbtController extends Controller
{
    public function index()
    {
        ...
        return view("ubt.index", compact("ubts"));
    }
    ...
    public function edit(string $id)
    {
        ...
        return view('ubt.editer', compact('demande', 'equipes', 'users', 'chefEquipeUsers'));
    }
    ...
    public function update(Request $request, string $id)
    {
        ...
        if ($demande->statut === 'cloture') {
            return redirect()->route('ubt.demandes.recues')->with('error', 'La demande ne peut plus être modifiée.');
        }
        ...
        // Mapper correctement le commentaire UBT (champ du formulaire => colonne comment_ubt)
        if ($request->has('commentaire')) {
            $updateData['comment_ubt'] = $request->input('commentaire');
            ...
            if ($request->has('equipes_noms_new') && $request->has('duree_new')) {
                foreach ($request->input('equipes_noms_new', []) as $index => $nomEquipe) {
                    if (!empty($nomEquipe) && ($request->input('duree_new.' . $index, 0) > 0)) {
                        ...
                        if ($equipe) {
                            $executantId = $request->input('executant_equipe_new.' . $index);
                            $demande->equipes()->syncWithoutDetaching([
                                $equipe->id => ['duree' => $request->input('duree_new.' . $index), 'executant_id' => $executantId ?: null]
                            ]);
                        }
                    }
                }
        ...
        switch ($action) {
            case 'dispatcher':
                if ($teamType === 'externe') {
                    // Prestataire externe : superviseur externe obligatoire, pas chef d'équipe
                    ...
                    return redirect()->route('ubt.demandes.validees')->with('success', 'Demande validée pour prestataire externe.');
                }
                ...
                if ($chefEquipeUserId) {
                    ...
                    return redirect()->route('ubt.demandes.validees')->with('success', 'Demande validée et transférée au chef d\'équipe.');
                }
            ...
            case 'retour':
                if ($demande->chef_equipe_id) {
                    ...
                    return redirect()->route('ubt.demandes.recues')->with('success', 'Demande renvoyée au chef d\'équipe.');
                }
            ...
            case 'debut_travaux':
                ...
                return redirect()->route('ubt.demandes.debutees')->with('success', 'Début des travaux enregistré.');
            ...
            case 'terminer':
                ...
                return redirect()->route('ubt.demandes.terminees')->with('success', 'Demande terminée avec succès.');
            ...
            case 'cloturer':
                ...
                try {
                    ...
                    foreach (['umt', 'ubt', 'unsp', 'umr', 'utgc'] as $u) {
                        if ($demande->{$u . '_id'}) { $n3 = User::find($demande->{$u . '_id'}); $n3Name = $demande->$u?->name; break; }
                    }
                    ...
                    \Log::error('Erreur lors de la génération/envoi du PDF de clôture UBT', [
                        'demande_id' => $demande->id,
                        'message'    => $e->getMessage(),
                    ]);
                }

                return redirect()->route('ubt.demandes.recues')->with('success', 'Demande clôturée avec succès.');

            default:
                return redirect()->route('ubt.demandes.recues')->with('success', 'Demande mise à jour avec succès.');
        }
    }
    ...
    public function demandesTerminees()
    {
        ...
        $demandes = Demande::where('statut', 'termine')
            ->where(function ($q) use ($user) {
                $q->where('ubt_id', $user->id)
                  ->orWhereHas('ubt', fn($s) => $s->where('id', $user->id));
            })
            ->with(['user', 'site', 'service', 'equipes', 'chefequipe', 'superviseur', 'executant', 'terminatedBy', 'validatedBy', 'ubt'])
            ->orderBy('updated_at', 'desc')->get();
        return view('ubt.demandes.terminees', compact('demandes'));
    }
    ...
    public function demandesCloturees()
    {
        ...
        $demandes = Demande::where('statut', 'cloture')
            ...
            ->with(['user', 'site', 'service', 'equipes', 'chefequipe', 'superviseur', 'executant', 'terminatedBy', 'cloturedBy', 'validatedBy', 'ubt'])
            ->orderBy('updated_at', 'desc')->get();
        return view('ubt.demandes.cloturees', compact('demandes'));
    }
    ...
    public function dashboard(Request $request)
    {
        ...
        $baseQuery = fn() => Demande::where(function ($q) use ($user) { $q->whereNotNull('ubt_id')->orWhere('ubt_id', $user->id); })
        ...
        $travauxDebutes = Demande::where('ubt_id', $user->id)->where('statut', 'en_cours')
            ->with(['user', 'site', 'service', 'chefequipe'])->orderBy('date_debut_intervention', 'desc')->limit(10)->get();

        $periodesAvenir = Demande::where('ubt_id', $user->id)
        ...
        // Préparer les compteurs individuels (pour x-dashboard-stats)
        ...
        return view('ubt.dashboard', compact(
            'totalDemandes',
            'statsParStatut',
            'demandesParMois',
    ...
    {
        $demandes = Demande::whereNotNull('ubt_id')
            ->where('statut', 'impute')
            ->orderBy('created_at', 'desc')
            ->get();
        foreach ($demandes as $d) $d->statut = ucfirst(str_replace('_', ' ', $d->statut));
        return view('ubt.demande', compact('demandes'));
    }

    public function demandesValidees()
    {
        $demandes = Demande::whereNotNull('ubt_id')->where('statut', 'valide')
            ->with(['user', 'service', 'site'])->orderBy('updated_at', 'desc')->get();
        return view('ubt.demandes_validees', compact('demandes'));
    }

    public function demandesDebutees()
    {
        $user = auth()->user();
        $demandes = Demande::where('ubt_id', $user->id)->where('statut', 'en_cours')
            ->with(['user', 'site', 'service', 'equipes', 'chefequipe', 'superviseur', 'executant'])
            ->orderBy('date_debut_intervention', 'desc')->get();
        return view('ubt.demandes.debutees', compact('demandes'));
    }
}
